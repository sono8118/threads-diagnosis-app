...
| **T4** | もったいないタイプ | 事業力が最低 | 発信は強いのに売上に変換できていない状態 |
| **BALANCED** | 安定成長タイプ | 全軸85点以上 | 続けるための土台がきれいに整っている状態 |

**判定ロジック**:
1. 全軸0点 → BEGINNER
2. 全軸85点以上 → BALANCED
3. それ以外 → 4軸スコアのうち最低スコアの軸でタイプ判定（T1-T4）
...
| **`CTA_View`** | **P-002** | **CTA表示** | **商品提案CTA表示（インプレッション）** + Type + Score |
...
```javascript
{
  diagnosis_type: 'BEGINNER' | 'T1' | 'T2' | 'T3' | 'T4' | 'BALANCED',  // タイプ（6種類）
...
https://example.com/register?
  ...
  utm_content=BEGINNER          # 動的：タイプ別（BEGINNER/T1/T2/T3/T4/BALANCED）
